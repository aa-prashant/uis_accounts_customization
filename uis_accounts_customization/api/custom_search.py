...
from frappe.desk.search import get_std_fields_list, relevance_sorter, build_for_autosuggest, search_widget as std_search_widget
...
def custom_search(
    ...
    if doctype in ["Customer", "Supplier"] and reference_doctype in ["Sales Order", "Quotation", "Purchase Order"] and filters:
        filters = json.loads(filters)
        if reference_doctype in ["Sales Order", "Purchase Order"] and doctype in ["Customer", "Supplier"]:
            filters = [["Party Account", "company", "=",  filters['company'][-1]]]
        elif reference_doctype == "Quotation":
            filters = [filters.get("company")]
